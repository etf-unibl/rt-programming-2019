...
static long gpio_driver_ioctl(struct file *, unsigned int cmd, unsigned long arg);
...
/*
 * Cleanup:
 *  1. Unmap GPIO Physical address space from virtual address
 *  2. Free buffer
...
/*
 * This function controls the device based on the cmd code provided as parameter.
 *
 */
static long gpio_driver_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    int wPi_pin = *(int*)arg;
    int pin = map_to_gpio_pinout(wPi_pin);
	...
	case PULLUP:
	    ...
	    break;
        case PULLNONE:
	    SetInternalPullUpDown(pin, PULL_NONE);
	default:
	    /* Error. */
	    return -1;
    }
...
/*
 * This function maps wiringPi pinout format to GPIO pinout format defined in this file.
 * 
 * Parameters:
 *         pin - int - pin number in wiringPi pinout format
 * Returns:
 *         int - pin number in GPIO format or -1 if wrong value is passed to function
 */
int map_to_gpio_pinout(int pin)
{
    int gpio_pins[30] = { GPIO_17, GPIO_18, GPIO_27, GPIO_22, GPIO_23, GPIO_24, GPIO_25, GPIO_04, GPIO_02, GPIO_03,
                          GPIO_08, GPIO_07, GPIO_10, GPIO_09, GPIO_11, GPIO_14, GPIO_15, -1, -1, -1, -1, GPIO_05,
			  GPIO_06, GPIO_13, GPIO_19, GPIO_26, GPIO_12, GPIO_16, GPIO_20, GPIO_21 };
