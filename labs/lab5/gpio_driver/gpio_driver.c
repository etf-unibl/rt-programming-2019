...
static long gpio_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
...
static long gpio_driver_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
	 gpio_pin_val p;
    ...
    switch (cmd)
    {
        case GPIO_PIN_GET:
			p=*(gpio_pin_val*)arg;
			p.direction = GetGpioPinDirection(p.pin);
			printk(KERN_INFO "GPIO Pin: %d    Direction: %d \n", p.pin, p.direction);
			if (copy_to_user((gpio_pin_val *)arg, &p, sizeof(gpio_pin_val)))
            {
                return -EACCES;
            }
            break;
        case GPIO_PIN_SET:
            if (copy_from_user(&p, (gpio_pin_val *)arg, sizeof(gpio_pin_val)))
            {
                return -EACCES;
            }
			printk(KERN_INFO "Gpio_PIN: %d    Dir: %d \n", p.pin, p.direction);
            SetGpioPinDirection(p.pin, p.direction);
            break;
        default:
            return -EINVAL;
    }
