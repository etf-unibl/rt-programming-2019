...
/*Definisemo i strukturu kojom cemo prebacivati 3 argumenta, broj pina, smijer(in/out) i stanje (pull up/pull down)*/
/*TODO napisati header za ove gluposti*/
typedef struct {
	uint32_t pin;
	uint32_t direction;
	uint32_t value;
...
struct file_operations gpio_driver_fops =
{
	...
	unlocked_ioctl : gpio_ioctl //dodao ovo
};
...
/*
	Funkcija koja vraca vrijednost smjera GPIO pina, kao parametar joj se prosljedjuje 
	vrijednost pina, ona uzima njegov ofset za registre i procita vrijednost smjera.
	vraca DIRECTION kao in ili out
*/
...
static long gpio_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        case PIN_SET:
			copy_from_user(&pa ,(pin_arg*) arg, sizeof(pa));
			SetGpioPinDirection(pa.pin, pa.direction);
			if(pa.direction==GPIO_DIRECTION_OUT)
				if(pa.value==1)
					SetGpioPin(pa.pin);
				else
					ClearGpioPin(pa.pin);
			printk(KERN_INFO "PIN %d has been set to with direction %s \n",pa.pin,(pa.direction==GPIO_DIRECTION_IN)?"in":"out");
			break;
        case PIN_READ:
			pa=*(pin_arg*)arg;
			pa.value=GetGpioPinValue(pa.pin);
			pa.direction=GetGpioPinDirection(pa.pin);
			copy_to_user((pin_arg*) arg, &pa, sizeof(pa));
            break;
        }
        return 0;
}
